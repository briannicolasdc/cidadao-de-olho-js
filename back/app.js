...
router.get('/deputados/:deputadoId/gastos', async (req, res) => {
    const gastoTotalDeputado = await Deputado.findOne({
        attributes: ['id', 'nome', 'gastoTotal'],
        where: { id: Number(req.params.deputadoId) },
    })

    console.log('gasto total', gastoTotalDeputado)
    res.send({
        gastoTotalDeputado,
    })
})
...
router.get('/gastos-totais', async (req, res) => {
    try {
        const gastosTotais = await Deputado.findAll({
            attributes: ['id', 'nome', 'gastoTotal'],
        })
